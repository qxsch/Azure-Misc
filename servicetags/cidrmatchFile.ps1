param(
    ...
    [string]$jsonFile = ".\ServiceTags_Public_20220822.json"
)
...
if($json.values -and $json.values -is [array]) {
    foreach($v in $json.values) {
        if(-not ($v.properties -and $v.properties.addressPrefixes -and $v.properties.addressPrefixes -is [array])) {
            break
        }
